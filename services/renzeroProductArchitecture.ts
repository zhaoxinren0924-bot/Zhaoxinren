...
const prdV5Requirements: PrdRequirementCoverage[] = [
  ...
  },
  {
    ...
    status: 'in-progress',
    evidence: ['EdgeConfidenceLogRecord stores old_confidence, new_confidence, n_verifications, and rule_applied.'],
    nextStep: 'Implement NaturalGradientUpdater that computes lr × p×(1-p) / n and applies REFUTED × -2.5 updates with 0.10/0.98 bounds.',
  },
  {
    id: 'V5-FAIL-001',
  ...
  },
  {
    ...
    status: 'in-progress',
    evidence: ['SourceTimingRecord stores earliest_signal_source, earliest_signal_ts, market_consensus_ts, lead_days, and topic_layer.'],
    nextStep: 'Write source timing records automatically from CONFIRMED verification events.',
  },
  {
    id: 'V5-SOURCE-002',
    part: 'Vector 4: Source Timing Calibration',
  ...
  },
  {
    ...
    status: 'not-started',
    evidence: ['Seed verification_results exist, but no verify writer exists.'],
    nextStep: 'Add verifyPrediction writer and CLI flags that update verification_results, edge_confidence_log, source_timing_records, and prediction_failure_records.',
  },
  {
    id: 'V5-OPS-001',
